...
if (!BatchAnalysisOn) {
  ...
  univar_batch_result_tb <- tibble(
    ...
    'AUC CV CI 95' = numeric(),
    'Odds Ratio' = numeric(),
    CI2.5 = numeric(),
    CI97.5 = numeric(),
    'Bayes AUC' = numeric(),
    'Bayes AUC CV CI 5' = numeric(),
    'Bayes AUC CV CI 50' = numeric(),
    'Bayes AUC CV CI 95' = numeric(),
    'Bayes Odds Ratio' = numeric(),
    'Bayes CI2.5' = numeric(),
    'Bayes CI97.5' = numeric(),
    'Bayes AUC HS' = numeric(),
    'Bayes HS AUC CV CI 5' = numeric(),
    'Bayes HS AUC CV CI 50' = numeric(),
    'Bayes HS AUC CV CI 95' = numeric(),
...
for (i in 1:(length(var_groups)) ) { 
  ...
  model_brm[[i]] <- brm(outcome ~ .,
                        data = brm_data,
                        family = bernoulli(), cores = parallel::detectCores(),
  ...
  for (jj in 1:repeats) {
    ...
    crossval.train.data <- brm_data[sample(nrow(brm_data)),]
    ...
    while (testSmp) {
      for (ii in 1:outsidefolds) {
        ...
        if ( length(unique(of_crossval.test.data$outcome)) == 1 ) { #
          ...
          ctr_smp = ctr_smp + 1
          if (ctr_smp > 100) { #if we can't resample easily just go ahead (modesl will be bad)
  ...
  if (!ctr_smp_flag) { #normal sample size
    univar_batch_result_tb[ctr,'AUC CV CI 5'] <- sort(auc)[round(length(auc)*.05)]
    univar_batch_result_tb[ctr,'AUC CV CI 50'] <- sort(auc)[round(length(auc)*.5)]
    univar_batch_result_tb[ctr,'AUC CV CI 95'] <- sort(auc)[round(length(auc)*.95)]
    
    univar_batch_result_tb[ctr,'Bayes AUC CV CI 5'] <- sort(auc_brm)[round(length(auc)*.05)]
    univar_batch_result_tb[ctr,'Bayes AUC CV CI 50'] <- sort(auc_brm)[round(length(auc)*.5)]
    univar_batch_result_tb[ctr,'Bayes AUC CV CI 95'] <- sort(auc_brm)[round(length(auc)*.95)]
    
    univar_batch_result_tb[ctr,'Bayes HS AUC CV CI 5'] <- sort(auc_brm_hs)[round(length(auc)*.05)]
    univar_batch_result_tb[ctr,'Bayes HS AUC CV CI 50'] <- sort(auc_brm_hs)[round(length(auc)*.5)]
    univar_batch_result_tb[ctr,'Bayes HS AUC CV CI 95'] <- sort(auc_brm_hs)[round(length(auc)*.95)]
  } else { # not enough samples for CV
    univar_batch_result_tb[ctr,'AUC CV CI 5'] <- NA
    univar_batch_result_tb[ctr,'AUC CV CI 95'] <- NA
  
    univar_batch_result_tb[ctr,'Bayes AUC CV CI 5'] <- NA
    univar_batch_result_tb[ctr,'Bayes AUC CV CI 95'] <- NA
  
    univar_batch_result_tb[ctr,'Bayes HS AUC CV CI 5'] <- NA
    univar_batch_result_tb[ctr,'Bayes HS AUC CV CI 95'] <- NA
  }  
