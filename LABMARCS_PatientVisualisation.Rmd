...
---
# Note R Studio has an issue where the setwd only applied to a chunk, to get around this first run the following on the command line
...
#create a store nunber of days from covid positive to first measure per patient
days_to_reading_mt = matrix(data = NA, nrow = dim(dem)[1], ncol = length(levels(total$Measure)))
...
covid_ls = matrix( data = TRUE, nrow = dim(dem)[1], ncol = 1 )

for (i in (1:dim(dem)[1])) { # 10)){ #
  
    #grab some data on patient and tests (copied from show_patient_viz)
    patientID = dem$uob_ID[i]
    positiveDate = (CovidCT %>% filter(CovidCT$uob_ID == patientID))[1,]$date
    val_outcome = subset(dem, uob_ID == patientID)[1,]
    new_data = total %>% filter(uob_ID == patientID)
    
    #check if they had a positive test
    if (!is.na(positiveDate) & (dem$Age[i] > 17) ) {
      ...
      for (j in unique_measures_ls) {
        ...
        days_between_pos_reading = new_data$date[idx1] - positiveDate
        ...
        if (sum(idx2) > 0) {
          ...
          num_readings_mt[i,j] = length(days_between_pos_reading)
          
        }
      }
      cat("### Visualisation of patient", i)
      cat("\n\n")
