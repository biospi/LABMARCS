...
# From 3 to skip age and gender: to length(fulldata)-1 to avoid altering the outcome variable
...
if(IncludeFirthBias==0) {
  # run through all variables excluding the outcome which is in the last position
  for (i in 1:(length(fulldata)-1)) {
    modelunivar[[i]] <- glm(fulldata$outcome ~ fulldata[,i]
                            , family = "binomial")
    summary(modelunivar[[i]])
    # Show ORs for the models and their confidence intervals
  ...
  # the outcome which is in the last position
  for (i in 3:(length(fulldata)-1)) {
...
# find AUC, brier score and plot ROC curve
...
for(j in 1:repeats) {
  ...
  for(i in 1:outsidefolds){
    ...
    # ORs (no CIs as innapropriate for LASSO)
...
# Currently there are numerous small bugs when running with co-morbidities turned on.
# I think due to the small sample size plus many extra variables.
if (Comorbidities==0) {
  # Stability analysis--------------------------------------------------------
  ...
  # RANKING OF THE P-VALUES MAY BE MORE USEFUL THAN ACTUAL VALUES.
...
# BUT I HAVE POPULATED WITH THE MINIMUM NEEDED TO CROSS COMPARE MODELS)
  ...
  saveRDS(globalmodel,file.path(mainDir, subDir, subsubDir, "globalmodel.rds"))
