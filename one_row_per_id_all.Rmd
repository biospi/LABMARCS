...
x <- c("ID", "BE_val")
colnames(BE2) <- x
BE2$BE_val <- as.character(BE2$BE_val)
...
x <- c("ID", "BNP_val")
colnames(BNP2) <- x
BNP2$BNP_val <- as.character(BNP2$BNP_val)
...
x <- c("ID", "CRP_val")
colnames(CRP2) <- x
CRP2$CRP_val <- as.character(CRP2$CRP_val)
...
DDM2$DDM_val <- as.character(DDM2$DDM_val)
...
eGFR2$eGFR_val <- as.character(eGFR2$eGFR_val)
...
FER2$FER_val <- as.character(FER2$FER_val)
...
fib2$fib_val <- as.character(fib2$fib_val)
...
Glucose2$Glucose_val <- as.character(Glucose2$Glucose_val)
...
HB2$HB_val <- as.character(HB2$HB_val)
...
HBA1c2$HBA1c_val <- as.character(HBA1c2$HBA1c_val)
...
LDH2$LDH_val <- as.character(LDH2$LDH_val)
...
PCT2$PCT_val <- as.character(PCT2$PCT_val)
...
PLT2$PLT_val <- as.character(PLT2$PLT_val)
...
trig2$trig_val <- as.character(trig2$trig_val)
...
trop2$trop_val <- as.character(trop2$trop_val)
...
FBCLymph2$Lymphocytes <- as.character(FBCLymph2$Lymphocytes)
...
FBCNeutr2$Neutrophils <- as.character(FBCNeutr2$Neutrophils)
...
FBCWCC2$WCC <- as.character(FBCWCC2$WCC)
...
FBCNLR2$NLR_val <- as.character(FBCNLR2$NLR_val)
...
ClotAPTT2$APTT_val <- as.character(ClotAPTT2$APTT_val)
...
ClotPT2$PT_val <- as.character(ClotPT2$PT_val)
...
poctLAC2$poctLAC_val <- as.character(poctLAC2$poctLAC_val)
...
O22$O2_val <- as.character(O22$O2_val)
...
CO22$CO2_val <- as.character(CO22$CO2_val)
...
poctpH2$poctpH_val <- as.character(poctpH2$poctpH_val)
...
AvonCap <- subset(AvonCap, select = -c(X,Repeat.Instrument,Repeat.Instance,
                                       Hospital,COVID.19.diagnosis))
AvonCap = AvonCap %>%
    rename(
...
AvonCap <- subset(AvonCap, select = c(ID,PSI,Ethnicity,Smoking.status,
...
totalBinary <- merge(total,AvonCap,by=c("ID"))
