...
sink(paste(save_path,'Patient_Numbers.txt'),sep = '')
site_ls = c('NBT','Weston','UHB1','UHB2')
for (site in site_ls) {
  print(paste('All', site, as.character(sum(fulldata$Site == site)) ))
  print(paste('Died',site, as.character(sum(fulldata$Site == site & fulldata$died))))
  print(paste('ICU', site, as.character(sum(fulldata$Site == site & fulldata$went_to_icu))))
}
sink()
...
if (ImputationAllowed == 0) {
  #list of rows with at least one missing value
...
print(paste('Dimensions full data',dim(fulldata)))
