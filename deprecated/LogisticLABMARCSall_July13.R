...
if (ImputationAllowed == 0) {
   #list of rows with at least one missing value (from manaul inspection)
   nan_indices <- c(5,7,11,15,18,19,20,21,23)
...
#get only cases without NAs
#for some reason this causes an error with factors?!
...
sink(paste(save_path, 'Univariate_and_AgeGender_Models.txt',sep = '')) #open text file to review later
for (i in 1:(dim(fulldata)[2] - 1)) { #last column has outcome so skip
...
for (j in 1:repeats) {
  ...
  for (i in 1:outsidefolds) {
    ...
    StatModel3 <- glmnet(x, train.data$outcome, alpha = 1, family = "binomial",
                         lambda = cv.lasso$lambda.1se) #lambda = lambda_1se_median) 
    #cv.lasso$lambda.1se) instead of using lambda estimated for each dataset
    #use our best guess from above that is median of set that were 1se above
    #the optimal lambda
...
sink(paste(save_path, 'Model_3_LassoCV_', model_desc_str, 'summary.txt',sep = '')) 
print("Mean AUC")
print(mean(auc))
print('Quantile')
...
pdf(paste(save_path, 'Model_3_LassoCV_', model_desc_str, 'AUC_Histogram.pdf',sep = ''))
...
pdf(paste(save_path, 'Model_3_LassoCV_',model_desc_str, 'BrierScore_Histogram.pdf',sep = ''))
hist(brier,plot = TRUE)
dev.off()

pdf(paste(save_path, 'Model_3_LassoCV_', model_desc_str, 'Lamda_Histogram.pdf',sep = ''))
...
ggsave(paste(save_path, 'Model_3_LassoCV_', model_desc_str, 'ROCs_for_',
...
if (Comorbidities == 0) {
  ...
  sink(paste(save_path, 'Model_3_LassoCV_',model_desc_str,
  ...
  pdf(paste(save_path, 'Model_3_LassoCV_', model_desc_str,
  ...
  pdf(file = paste(save_path, 'Model_3_LassoCV_', model_desc_str, 
...
pdf(paste(save_path, 'Model_4_LassoAllData_Lambda.pdf',sep = ''))
plot(cv.lasso)
dev.off()
...
sink(paste(save_path, 'Model_4_LassoAllData_Summary.txt',sep = ''))
...
ggroc(roccurve3, legacy.axes = T) +
  ...
  scale_x_continuous(breaks = seq(0,1,0.25), labels = seq(0,1,0.25)) + 
  scale_y_continuous(breaks = seq(0,1,0.25), labels = seq(0,1,0.25))
ggsave(paste(save_path, 'Model_4_LassoAllData_ROC.pdf', sep = ''),device = 'pdf',
       width = 20, height = 20, units = 'cm', dpi = 300)
