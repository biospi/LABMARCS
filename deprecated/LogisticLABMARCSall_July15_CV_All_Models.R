...
#setwd("Z:/brian/data/")
setwd("C:/Users/bs16044/OneDrive - University of Bristol/HDR-UK-AMR/LABMARCS/brian/data/")

save_path = paste(getwd(),'/output/Day-',dateRange,'_Reading-',readingwanted_str,
...
#our data don't show this?
#C02 and BE (bicarbonate excess are correlated) - C02 not taken often BE
...
#library('writexl')
#write_xlsx(fulldata, path = tempfile(pattern = 'fulldata', fileext = ".xlsx"))  

#Below are variables that have 30% or more data missing NA/not taken
# [1] "BE_val, 3, 0.00,53.59"
# [1] "BNP_val, 4, 0.00,96.05"
# [1] "DDM_val, 6, 0.00,91.99"
# [1] "eGFR_val, 7, 39.80,NA"
# [1] "FER_val, 8, 0.00,91.39"
# [1] "fib_val, 9, 0.00,90.07"
# [1] "Glucose_val, 10, 0.00,76.20"
# [1] "HBA1c_val, 12, 0.00,98.21"
# [1] "LDH_val, 13, 0.00,94.74"
# [1] "PCT_val, 14, 0.00,97.61"
# [1] "trig_val, 16, 0.00,99.28"
# [1] "trop_val, 17, 0.00,83.49"
# [1] "APTT_val, 22, 0.00,56.10"
# [1] "poctLAC_val, 24, 0.00,87.08"
# [1] "O2_val, 25, 0.00,87.20"
# [1] "CO2_val, 26, 0.00,87.08"
# [1] "poctpH_val, 27, 0.00,53.59"

if (1) { #use to examine proportions of data missing
  fulldata_sv <- fulldata
  ...
  for (i in 3:dim(fulldata_sv)[2]) {
    ...
    if ((!is.na(na_perc1) & na_perc1 > perc_thresh) | 
      ...
      str <- sprintf('%d, %s, %2.2f, %2.2f, %2.2f', i, names(fulldata_sv)[i], nt_perc,na_perc1, na_perc2)
      print(str)
  ...
  fulldata <- fulldata_sv[,name_tmp]
  print( sum(complete.cases(fulldata)))
  print(dim(fulldata))
...
#remove rows with missing data - complete.cases function does this instead
#however it seems to differ slightly and create factor error try this for now
if (ImputationAllowed == 0 & FALSE) {
   #list of rows with at least one missing value (from manual inspection)
   nan_indices <- seq(3,dim(fulldata)[2])
   tmp <- fulldata[, nan_indices]
   tmp <- apply(tmp, 2, is.na) #list of NA entries
   complete_entry_idx = apply(tmp, 1, any) #save !NAs
   siteData <- siteData[!complete_entry_idx] #use for test/train separation
   fulldata <- fulldata[!complete_entry_idx,]
   print(dim(fulldata))
}

#Some of the columns don't have enough examples to allow models to converge
#may need to omit some people with infrequent examples? These names are from
#using 10% for test data
#fulldata <- subset(fulldata, select = -c(fib_val, LDH_val, Neutrophils, 
#                                         WCC, APTT_val, bc_coinfection, trig_val ))
#USE ABOVE ONLY FOR TESTING SHOULDNT OMIT COLUMNS YET

site_ls <- c('NBT','Weston', 'UHB')
if (1) {
  sink(paste(save_path,'filtered_variable_histogram_counts.txt'))
...
source("create_dummy_var_list.r")
...
if (ImputationAllowed == 0) {
  # Keep only complete cases  this should be redundant but for some reason
  #if we don't do glm complains about new factors
  ...
  #plot pattern of missing data, default formatting is terrible revisit to make
  #readable
  #md.pattern(fulldata)
...
print("Run Model Cross Validation for Lasso")
model_desc_str = paste('R',as.character(repeats),'_OF', 
...
for (mnum in mod_ls) { 
  ...
  pdf(paste(save_path,  'Model_CV_', mnum, '_', model_desc_str, 'Lamda_Histogram.pdf',sep = ''))
  ...
  #---------------------------
  # Stability analysis-using data gather from above nested folding
  #---------------------------
  print("Run Stability Analysis for Lasso")
...
write.table(batch_df, file = paste(getwd(),'/output/batch_summary_table.txt',sep=''),
            row.names = FALSE)
