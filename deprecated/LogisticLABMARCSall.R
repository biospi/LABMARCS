...
# Choose to include Firth's bias in the models (constrains parameters from >> numbers)
#currently breaks - debug
IncludeFirthBias <- 0
  
# Use knn imputation==1, MICE==2, None==0 If none, complete case analysis is used
ImputationAllowed <- 1
...
# Plot histograms of continuous variables to examine distributions
pdf("age_histogram_transformed.pdf")
nbreaks <- pretty(range(fulldata$Age),n = 20)
par(mfrow = c(1,1))
xlab <- paste('Age, bin width=', as.character(nbreaks[2] - nbreaks[1]))
hist(fulldata$Age,main = "Distribution of Patient Ages",xlab = xlab, breaks = nbreaks)
...
# Plot histograms of continuous variables to examine distributions
pdf("age_histogram_regularized.pdf")
nbreaks <- pretty(range(fulldata$Age),n = 20)
par(mfrow = c(1,1))
xlab <- paste('Age, bin width=', as.character(nbreaks[2] - nbreaks[1]))
hist(fulldata$Age,main = "Distribution of Patient Ages",xlab = xlab, breaks = nbreaks)
...
#Original code was omitting one varibale by mistake and R console was not printing 
...
sink('Simple_Preliminary_Models.txt') #open text file to review later
...
for (i in 1:(dim(fulldata)[2])) {
  modeldemo[[i]] <-
    glm(
      fulldata$outcome ~ fulldata[, i] + fulldata$Age + fulldata$Gender,
      family = "binomial",
      method = family_ls[IncludeFirthBias + 1]
    )
  print('--------------------------------------------------------------------')
...
for (i in c(1,2)){
  ...
  sink(paste('Model_',as.character(i),'_summary.txt',sep='')) #open text file to review later
  ...
  adjustedp <- p.adjust(coef(summary(model))[,4], method = "fdr",
                         n = length(coef(summary(model))[,4]))
  ...
  probabilities <- predict(object = model, test.data, type = "response")
  ...
  pdf(paste('Model_',as.character(i),'_roc.pdf',sep=''))
...
sink(paste('Model_3_glm_lasso_cross-validation.txt',sep='')) 
print("Mean AUC")
print(mean(auc))
print('Quantile')
...
pdf(paste('Model_3_AUC_Histogram.pdf',sep=''))
hist(auc,plot=TRUE)
dev.off()

pdf(paste('Model_3_BrierScore_Histogram.pdf',sep=''))
hist(brier,plot=TRUE)
dev.off()

pdf(paste('Model_3_Lambda_Score_Histogram.pdf',sep=''))
hist(lambda.store,plot=TRUE)
dev.off()

# plot the ROC curves for all models to show uncertainty
pdf(paste('Model_3_AUCs_Histogram.pdf',sep=''))
